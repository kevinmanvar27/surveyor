...
/// Provider to get active surveys (Working + Waiting, NOT Done) for expense linking
final activeSurveysForExpenseProvider = FutureProvider<List<SurveyModel>>((ref) async {
  if (AppConfig.useDemoMode) {
    final allSurveys = DemoSurveyData.getSurveys();
    return allSurveys.where((s) => s.status != SurveyStatus.done).toList();
  }
  final repository = ref.watch(surveyRepositoryProvider);
  if (repository == null) {
    final allSurveys = DemoSurveyData.getSurveys();
  ...
  // Get all surveys and filter out Done status
  final allSurveys = await repository.getSurveys(
    userId: userId,
...
/// Provider to get waiting surveys for expense linking (legacy - use activeSurveysForExpenseProvider instead)
final waitingSurveysProvider = FutureProvider<List<SurveyModel>>((ref) async {
  return ref.watch(activeSurveysForExpenseProvider.future);
});
