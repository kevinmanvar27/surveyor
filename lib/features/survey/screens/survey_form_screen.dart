...
class _SurveyFormScreenState extends ConsumerState<SurveyFormScreen>
  ...
  List<PaymentReceiptModel> _paymentReceipts = [];
  ...
  void _populateForm(SurveyModel survey) {
    ...
    _paymentReceipts = List.from(survey.paymentReceipts); // Load payment receipts
    // Calculate received payment from receipts
    _updateReceivedFromReceipts();
  }

  void _updateReceivedFromReceipts() {
    final totalReceived = _paymentReceipts.fold(0.0, (total, receipt) => total + receipt.amount);
    _receivedPaymentController.text = totalReceived.toStringAsFixed(0);
  }
  ...
  Future<void> _handleSave() async {
    ...
    final survey = SurveyModel(
      ...
      paymentReceipts: _paymentReceipts, // Include payment receipts
    );
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '₹${_receivedPaymentController.text.isEmpty ? "0" : _receivedPaymentController.text}',
                      style: TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.w700,
        ...
        Row(
          ...
          children: [
            ...
            TextButton.icon(
              onPressed: _showAddPaymentReceiptDialog,
        ...
        if (_paymentReceipts.isEmpty)
          ...
          ...List.generate(_paymentReceipts.length, (index) {
            final receipt = _paymentReceipts[index];
            return _buildPaymentReceiptItem(receipt, index);
          }),
  ...
  Widget _buildPaymentReceiptItem(PaymentReceiptModel receipt, int index) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  dateFormat.format(receipt.receivedDate),
                  style: TextStyle(
                    fontSize: 12,
                    color: AppColors.textSecondary,
                  ),
          ...
          IconButton(
            onPressed: () => _deletePaymentReceipt(index),
            icon: Icon(
  ...
  void _showAddPaymentReceiptDialog() {
        ...
        content: Form(
          ...
          child: TextFormField(
            ...
            validator: (value) {
              ...
              if (amount > _pendingPayment) {
                return 'Amount exceeds pending (₹${_pendingPayment.toStringAsFixed(0)})';
              }
              return null;
            },
          ),
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState!.validate()) {
                ...
                _addPaymentReceipt(amount, DateTime.now(), null);
  ...
  void _addPaymentReceipt(double amount, DateTime date, String? notes) {
    setState(() {
      _paymentReceipts.add(PaymentReceiptModel(
      ...
      _paymentReceipts.sort((a, b) => b.receivedDate.compareTo(a.receivedDate));
      _updateReceivedFromReceipts();
    });
  }

  void _deletePaymentReceipt(int index) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        ...
        content: Text(
          'Are you sure you want to delete this payment of ₹${_paymentReceipts[index].amount.toStringAsFixed(0)}?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
          ...
          ElevatedButton(
            onPressed: () {
              setState(() {
                _paymentReceipts.removeAt(index);
                _updateReceivedFromReceipts();
              });
              Navigator.pop(context);
            },
  ...
  Widget _buildExpensesSection(AppLocalizations loc) {
    ...
    // Use Consumer to ensure proper rebuilds when provider is invalidated
    return Consumer(
      ...
      child: Row(
        children: [
          ...
          Container(
            padding: const EdgeInsets.all(8),
            decoration: BoxDecoration(
              color: _getCategoryColor(expense.category).withOpacity(isDark ? 0.2 : 0.1),
              borderRadius: BorderRadius.circular(10),
            ),
            child: Icon(
  ...
  void _showAddExpenseDialog() {
    showModalBottomSheet(
      ...
      // Force refresh the expenses list - use refresh instead of invalidate
      // to ensure the provider re-fetches immediately
