...
class _ExpenseFormDialogState extends ConsumerState<ExpenseFormDialog> {
  ...
  // Check if survey is pre-selected (opened from survey edit screen)
  bool get isSurveyPreSelected => widget.surveyId != null;
  ...
  Widget _buildSurveyDropdown(AppLocalizations loc) {
    // If survey is pre-selected (from survey edit screen), show read-only display
    if (isSurveyPreSelected) {
      return _buildPreSelectedSurveyDisplay(loc);
    }
    
    // Otherwise show the dropdown for user to select
    final activeSurveysAsync = ref.watch(activeSurveysForExpenseProvider);
    ...
    return Column(
      ...
      children: [
        ...
        activeSurveysAsync.when(
          data: (surveys) {
            if (surveys.isEmpty && _selectedSurveyId == null) {
              return Container(
            ...
            // If selected survey is not in the list (e.g., it's now Done), clear selection
            final selectedInList = _selectedSurveyId == null || 
                surveys.any((s) => s.id == _selectedSurveyId);
            final effectiveSelectedId = selectedInList ? _selectedSurveyId : null;
            if (!selectedInList && _selectedSurveyId != null) {
              // Clear invalid selection on next frame
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted) setState(() => _selectedSurveyId = null);
              });
            }
  ...
  /// Build a read-only display showing the pre-selected survey
  Widget _buildPreSelectedSurveyDisplay(AppLocalizations loc) {
    // Get the survey details for display
